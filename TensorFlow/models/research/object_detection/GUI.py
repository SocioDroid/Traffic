...
class MainWindow:
    ...
    def __init__(self, master):
        ...
        self.lstatus.grid(row=3, columnspan=3)

        self.framenoplate = Frame(self.master, bg="gray", width=300, height=50)
        self.framenoplate.grid(row=2)
        self.framenoplate.grid_propagate(False)

        self.lplate = Label(self.framenoplate, bg="black", fg="red", font=("Ubuntu 28 bold"))
        self.lplate.pack(fill=BOTH)
    ...
    def processn(self):
        ...
        if not self.filename:
            return
        os.system("python Object_detection_video.py "+self.filename)
        with open('out.txt', 'r') as f:
            plate = f.readline()
            self.lplate.configure(text=plate)
        self.lstatus.configure(text="processing video.....", anchor=W)
        time.sleep(5)
        self.cap1 = cv2.VideoCapture(self.filename)
        self.cap2 = cv2.VideoCapture("testoutput.avi")
